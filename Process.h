...
/*
Process class that models a real process
Up to 26 processes can run at once, each with a letter id
Processes are randomly assigned CPU burst times (running instructions normally) and IO burst times (user interaction)
IO burst times are on average 10 times longer than CPU burst times
Once a process decrements all of its burst times to 0, it is removed from the queue
*/
